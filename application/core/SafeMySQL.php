 ...
 * ?h ("string")  - strings Оставляет все теги, кроме <script> <iframe>
...
class SafeMySQL {
    ...
    private $defaults = array(
        ...
        'pconnect' => FALSE,
        'charset' => 'utf8',
        'errmode' => 'error', // error or exception
    ...
    private function escapeHTML($value) {
        ...
        $value = preg_replace(array('#<script(.*?)>(.*?)</script>#is', '@<iframe[^>]*?>.*?</iframe>@siu'), array('', ''), $value);
        return "'" . mysqli_real_escape_string($this->conn, $value) . "'";
    }
    ...
    private function error($err) {
        ...
        if ($this->emode == 'error') {
            ...
            $str = "\n\nStart: {$start}\n";
            $str .= preg_replace_callback(array('/(select|insert into|update|from|where|order by|group by|inner join|on|as)/i'), function ($matches) {
                return strtoupper($matches[0]);
            }, $err);
            file_put_contents(__DIR__ . '/log/log-' . date('Y-m-d') . '.log', $str . ";\n\n", FILE_APPEND);
